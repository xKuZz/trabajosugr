...
bloque : ABRE_LLAVE {insertarMarca();}
	     declar_de_variables_locales 
	     declar_de_subprogs 
	     sentencias 
	     CIERRA_LLAVE {vaciarEntradas();};
...
lista_id : IDENTIFICADOR {insertarIdentificador($1);} | lista_id COMA IDENTIFICADOR {insertarIdentificador($3);} | lista_id error IDENTIFICADOR;
...
sentencia_asignacion : IDENTIFICADOR ASIGNACION expresion PUNTO_COMA { comprobarAsignacion($1, $3);};

sentencia_if : SI ABRE_PAREN expresion CIERRA_PAREN sentencia {comprobarBooleanoLoop($3);} |
			   SI ABRE_PAREN expresion CIERRA_PAREN sentencia SINO sentencia {comprobarBooleanoLoop($3);};

sentencia_while: MIENTRAS ABRE_PAREN expresion CIERRA_PAREN sentencia {comprobarBooleanoLoop($3);};

sentencia_entrada: LEER lista_leer PUNTO_COMA;

lista_leer: IDENTIFICADOR  | lista_leer COMA IDENTIFICADOR | lista_leer error IDENTIFICADOR;
...
sentencia_repeat_until : REPETIR sentencia HASTA ABRE_PAREN expresion CIERRA_PAREN PUNTO_COMA {comprobarBooleanoLoop($5);};
...
expresion: ABRE_PAREN expresion CIERRA_PAREN {$$.tipo = $2.tipo; $$.tipo_interno = $2.tipo_interno;}
		 | MAS_MENOS expresion %prec OP_UNARIO {comprobarTipoUnarioMasMenos($1, $2, &$$);}
		 | OP_UNARIO expresion {comprobarTipoUnario($1, $2, &$$);}
		 | expresion OP_TERNARIO1 expresion ARROBA expresion {comprobarTipoTernario($1, $3, $5, &$$);}
		 | expresion ARROBA expresion %prec OP_BINARIO {comprobarTipoArroba($1, $3, &$$);}
		 | expresion OP_BINARIO expresion {comprobarTipoBinario($2, $1, $3, &$$);}
		 | expresion OP_RELACIONAL_IGUALDAD expresion {comprobarTipoIgualdad($2, $1, $3, &$$);}
		 | expresion OP_RELACIONAL_AND expresion {comprobarTipoAnd($1, $3, &$$);}
		 | expresion OP_RELACIONAL_OR expresion {comprobarTipoOr($1, $3, &$$);}
		 | expresion OP_RELACIONAL expresion {comprobarTipoRelacional($2, $1, $3, &$$);}
		 | expresion MAS_MENOS expresion %prec OP_BINARIO {comprobarTipoMasMenos($2, $1, $3, &$$);}
