...
declar_subprograma : cabecera_subprograma {subProg = 1;} 
					 bloque {subProg = 0; --contador_subprogramas; volverAArchivoCompilado(); };
...
cabecera_subprograma : tipo_t IDENTIFICADOR { ++contador_subprogramas; cambiarAArchivoFunciones(); ponerCabeceraFuncion($1, $2); insertarFuncion($2);} ABRE_PAREN lista_parametros CIERRA_PAREN { fprintf(archivo_compilado, ")"); } ;

lista_parametros : parametro | lista_parametros COMA {fprintf(archivo_compilado, ",");} parametro | lista_parametros error IDENTIFICADOR |;

parametro : tipo_t IDENTIFICADOR {insertarParametros($2); declararTipo($1), fprintf(archivo_compilado, "%s", $2.lexema);};
...
sentencia_return_comienzo_lista: DEVOLVER_DOLAR expresion PUNTO_COMA {comprobarReturnDolar($1,$2); if ($1.atrib == 0) comienzolista($2); else sentenciareturn($2);};

avanzar_retroceder_lista: expresion DESPLAZA PUNTO_COMA { comprobarTipoDesplaza($2, $1); moverposicionlista($2, $1);};
...
expresion: ABRE_PAREN expresion CIERRA_PAREN {$$.tipo = $2.tipo; $$.tipo_interno = $2.tipo_interno;}
		 ...
		 | OP_UNARIO expresion {comprobarTipoUnario($1, $2, &$$); generarUnario($1, $2, &$$);}
		 | expresion OP_TERNARIO1 expresion ARROBA expresion %prec OP_TERNARIO1 {comprobarTipoTernario($1, $3, $5, &$$); generarTernario($1, $3, $5);}
		 | expresion ARROBA expresion %prec OP_BINARIO {atributos arroba; arroba.lexema="@"; comprobarTipoArroba($1, $3, &$$); generarBinario(arroba, $1, $3, &$$);}
...
funcion : IDENTIFICADOR ABRE_PAREN lista_f CIERRA_PAREN {comprobarFuncion($1, &$$); limpiarMarcaFuncion(); 
		...
		| IDENTIFICADOR ABRE_PAREN CIERRA_PAREN {insertarMarcaFuncion(); comprobarFuncion($1, &$$); limpiarMarcaFuncion();
		  ...
		  parentesis.lexema = "()";
		  ampliarLexema($1, parentesis, &$$);
		  };

tipo_t: TIPO {TipoTmp = $1.tipo; $$.tipo = $1.tipo;} | LISTA_DE TIPO {TipoTmp = $1.tipo; TipoTmp2 = $2.tipo; $$.tipo = $1.tipo; $$.tipo_interno = $2.tipo;};

agregado_lista : ABRE_CORCHETE lista CIERRA_CORCHETE {$$.tipo = $2.tipo; $$.tipo_interno = $2.tipo_interno; $$.lexema = $2.lexema;};

lista_f : expresion {insertarMarcaFuncion(); guardarParametro($1);} | lista_f COMA expresion {guardarParametro($3);
																			 atributos coma; 
																			 coma.lexema = ", " ;
																			 ampliarLexema($1, coma, &$$);
																			 ampliarLexema($$, $3, &$$);
																			};
