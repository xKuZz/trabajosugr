...
#' 
#' Vemos que regresión logística a parado rápidamente en un mínimo local y los errores son muy elevados. La selección de la diferencia de pesos entre épocas con un umbral como única condición ha sido una mala selección de condición de parada. Para poder mejorarlo podríamos, por ejemplo, haber puesto un máximo de iteraciones y añadir junto a la condición que puesta que el error Ein fuese menor que una cantidad deseada.
#' 
...
#' Vamos a poner los 4 con etiqueta 1 (en color verde) y los 8 con etiqueta -1 (en color cian).
#' 
...
error_clasificacion = function(w, datos, etiquetasOriginales) {
  ...
  etiquetasNuevas = apply(X=datos[,2:ncol(datos)],MARGIN=1, FUN = etiquetar_regresion, w = w)
