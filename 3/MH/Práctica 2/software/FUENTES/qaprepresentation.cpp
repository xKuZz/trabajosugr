...
long QAPRepresentation::getCostGRASP(const InputData& input, const int stream, const int distance)
{
    long new_costo=0;
    //auto preserve_permutation = permutation;
    for (int i = 0; i < input.size();++i)
        if(permutation[i] != -1)
            new_costo += input.getDistance(permutation[i], distance)*
                         input.getStream(i, stream);

    return new_costo;
//    long check_costo = new_costo;
//    assign(stream,distance);
//    new_costo = 0;
//    for (int i = 0; i < input.size(); ++i)
//        for (int j = 0; j < input.size(); ++j)
//            if (permutation[i] != -1 && permutation[j] != -1)
//            new_costo += input.getDistance(permutation[i], permutation[j]) *
//                    input.getStream(i,j);

//    permutation = preserve_permutation;

//    assert(check_costo == new_costo);
//    return new_costo;
}
