...
int main(int argc, char * argv[])
{
  ...
  for (int i = 0; i < n; i++)
    {
      T[i] = random();
		...
		tdespues = clock();
		tiempo_transcurrido=((double)(tdespues-tantes)/(CLOCKS_PER_SEC* (double)n));
	
  // escribe_vector(T, n); NO EXISTE
  //Captura el valor del reloj antes de la llamada a heapsort
	
	
  //Captura el valor del reloj despues de la ejecucion de heapsort
  ...
  // escribe_vector(T, n);
	//std::cout <<  transcurrido.count() ;
	cout << n << " " << (double)tiempo_transcurrido << endl;
