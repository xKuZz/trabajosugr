...
void GUI::myGlutTimer(int x){
    if (partida_remota){
        if (esperando_server){
          ...
          if (!env->JuegoTerminado()){
              //Recibir Accion
              cout << "Esperando la accion del " << nick_adversario <<endl;
	...
	switch(id){
		...
		case SELECTMAP_CANCEL_BTN_ID:
			...
			if (modo==2){ // En red yo en modo cliente
                ...
                cout << "Recibiendo en nick del servidor..." << endl;
                Cliente.Recibir_Msg(nick_adversario);
                cout << "Nick Adversario: " << nick_adversario << endl;
                cout << "Recibiendo el jugador que soy..." << endl;
                ...
                cout << "Esperando modo de juego\n";
                Servidor.Recibir_Accion();

                cout << "Enviar mi nick al adversario..." << endl;
                Servidor.Enviar_Cliente_Msg(mi_nick);

                cout << "Empezamos las partida..." << endl;
                cout << "Le indico al cliente que jugador es..." << endl;
                if (jugador==0)
                ...
                cout << "IP del servidor: " << field_ip_ninja << endl;
