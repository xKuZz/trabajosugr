#include "bintree.h" // AUTOCOMPLETAR
//-*-Mode: C++;-*-
          ...
          bintree<T> & orig)
{
  ...
  destroy(orig.laraiz);
  orig.num_nodos = count(n.left());
  num_nodos = num_nodos - orig.num_nodos;
  orig.laraiz = n.left();
  if (!orig.laraiz.null())
...
bintree<T>::inorder_iterator::operator--() {
  if (elnodo.null()) {
      elnodo = ptr->laraiz;
      while (!elnodo.right().null())
        elnodo = elnodo.right();
  }
  else if (!elnodo.left().null()) {
    elnodo = elnodo.left();
    while (!elnodo.right().null())
      elnodo = elnodo.right();
  }
...
{
  auto it = inorder_iterator(typename bintree<T>::node());
  it.ptr = this;
  return it;
}
